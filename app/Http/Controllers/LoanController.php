...
class LoanController extends Controller
{
    ...
    public function memberStore(Request $request): JsonResponse
    {
        ...
        // Validasi apakah member yang login sesuai dengan yang ada di request
        $memberId = $request->input('member_id');
        // Cek Member berdasarkan id
        $member = Member::find($memberId);
        if (!$member) {
            return response()->json(['message' => 'Member not found'], 404);
        }
        ...
        // Cek apakah buku sedang dipinjam oleh member yang sama dan belum dikembalikan
        $existingLoan = Loan::where('member_id', $memberId)
            ->where('book_id', $request->book_id)
            ->whereNull('return_date')
            ->first();

        if ($existingLoan) {
            return response()->json(['message' => 'Book is already borrowed by this member'], 400);
        }
